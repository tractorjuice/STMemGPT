...
def get_ai_reply_async(
    ...
    st.sidebar.write("Converting OpenAI to Langchain")
    st.sidebar.write(convert_openai_messages(message_sequence))
...
class AgentAsync(object):
    ...
    def summarize_messages_inplace(self, cutoff=None):
        ...
        st.sidebar.write(f"Attempting to summarize {len(message_sequence_to_summarize)} messages [1:{cutoff}] of {len(self.messages)}")
